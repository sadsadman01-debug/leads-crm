...
export function MfaResetRequestsPage() {
  const [tab, setTab] = useState<Tab>('pending')
  const [orgFilter, setOrgFilter] = useState('')
  const [resolving, setResolving] = useState<MfaResetRequest | null>(null)

  const { data, isLoading } = useQuery({ queryKey: ['mfa-reset-requests'], queryFn: mfaResetRequestsApi.list })
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-base-100">MFA Reset Requests</h1>
        <p className="mt-1 text-sm text-base-400">
          Platform-wide — every "Locked out of Two-Factor Authentication?" submission, from any Organization's Users
          or from any Admin.
        </p>
      </div>
      ...
      <MfaResetResolveModal request={resolving} onClose={() => setResolving(null)} />
    </div>
  )
}
