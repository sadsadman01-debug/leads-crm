...
import { formatPromoDiscount, type PromoCode, type PromoCodeDiscountType } from '@/types/promoCode'
...
function PromoCodeFormModal({ promoCode, onClose }: { promoCode: PromoCode | null; onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      isEdit
        ? promoCodesApi.update(promoCode!.id, { discount_type: discountType, discount_value: Number(discountValue) })
        : promoCodesApi.create({ code: code.trim().toUpperCase(), discount_type: discountType, discount_value: Number(discountValue) }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['promo-codes'] })
      onClose()
    },
